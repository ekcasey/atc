...
func (s *Server) ListJobBuilds(pipelineDB db.PipelineDB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if pagination.Next != nil {
			s.addNextLink(w, r.FormValue(":team_name"), pipelineDB.GetPipelineName(), jobName, *pagination.Next)
		}

		if pagination.Previous != nil {
			s.addPreviousLink(w, r.FormValue(":team_name"), pipelineDB.GetPipelineName(), jobName, *pagination.Previous)
		}
...
func (s *Server) addNextLink(w http.ResponseWriter, teamName string, pipelineName string, jobName string, page db.Page) {
...
func (s *Server) addPreviousLink(w http.ResponseWriter, teamName string, pipelineName string, jobName string, page db.Page) {
	w.Header().Add("Link", fmt.Sprintf(
		`<%s/api/v1/teams/%s/pipelines/%s/jobs/%s/builds?%s=%d&%s=%d>; rel="%s"`,
		s.externalURL,
		teamName,
