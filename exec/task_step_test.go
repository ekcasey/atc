			...
			Context("when getting the config works", func() {
				...
				Context("when a single worker can be located", func() {
					...
					Context("when creating the container fails", func() {
						...
						Context("on an WorkerNotCreatedError error", func() {
							BeforeEach(func() {
								fakeWorker.CreateContainerReturns(nil, atc.WorkerNotCreatedError{errors.New("nope")})
							})
					...
					Context("when the configuration has inputs", func() {
						...
						Context("when all inputs are present in the in source repository", func() {
							...
							Context("and some workers have more matching input volumes than others", func() {
								...
								Context("when creating the container on worker with the most volumes fails with WorkerNotCreatedError error", func() {
									disaster := atc.WorkerNotCreatedError{errors.New("nope")}
